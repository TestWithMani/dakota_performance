...
def sendEmailNotification(String buildStatus) {
    ...
    def timeoutValues = []
    cleanedFailedTests.each { item ->
        def m = (item =~ /(?i)\bexceeded\s+(\d+)\s*s\b/)
        if (m.find()) {
            timeoutValues << (m.group(1) as int)
        }
    }
    timeoutValues = timeoutValues.unique().sort()
    def timeoutHint = timeoutValues.isEmpty()
        ? 'Includes timeout and assertion failures.'
        : "Timeout threshold observed: ${timeoutValues.join(', ')} s."
    def failedSectionTitle = "Failed Tests / Affected Tabs (${timeoutHint})"
          ...
          <tr>
            <td style="padding:24px 30px 10px;">
              ...
              <table width="100%" cellpadding="0" cellspacing="0" style="font-size:14px;color:#1e293b;border:1px solid #c7d2fe;border-radius:10px;overflow:hidden;background:linear-gradient(180deg,#f8faff 0%,#fdfdff 100%);table-layout:fixed;">
                <tr>
                  <td width="32%" style="padding:10px 12px;background:linear-gradient(180deg,#e0e7ff 0%,#eef2ff 100%);border-bottom:1px solid #c7d2fe;"><strong>${failedSectionTitle}</strong></td>
